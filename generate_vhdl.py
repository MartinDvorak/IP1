...
def decoder(alphabet):
	...
	for letter in alphabet:
		decoder = decoder + """
		symb_decoder(16#"""+str(hex(ord(letter)))[2::]+"""#) <= '1' when (INPUT = X\""""+str(hex(ord(letter)))[2::]+"""\") else
                          '0';""" 
	decoder = decoder + "\n\n"
	return decoder
...
def set_logic(name,reverse_trans,start_state):
	...
	for item in reverse_trans:
		logic = logic + "(reg_"+str(item[1])+" AND symb_decoder(16#"+str(hex(ord(item[0])))[2::]+"#)) OR\n 					"
...
if __name__ == '__main__':
	...
	#state, com_rel, rel, trans,final_l,initial_l = call_intersection(fd_in) 
	#print(state)
